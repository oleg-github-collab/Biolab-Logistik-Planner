...
-- Message mentions table already created in migration 004
-- Adding position columns if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_name='message_mentions' AND column_name='start_position') THEN
    ALTER TABLE message_mentions ADD COLUMN start_position INTEGER;
    ALTER TABLE message_mentions ADD COLUMN end_position INTEGER;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_task_pool_help_requests ON task_help_requests(requested_user_id, status);
