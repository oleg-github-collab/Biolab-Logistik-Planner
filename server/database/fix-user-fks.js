...
async function fixAllUserFKs(pool) {
  ...
  try {
    ...
    // FIX THE TRIGGER THAT CAUSES THE ISSUE
    logger.info('🔧 Fixing audit trigger to handle user deletion...');
    try {
      await client.query(`
        ...
        BEGIN
          ...
          IF TG_OP = 'DELETE' THEN
            ...
            -- Skip audit logging if user is being deleted
        ...
        $$ LANGUAGE plpgsql;
      `);
      logger.info('✅ Audit trigger fixed - now handles user deletion safely');
    } catch (triggerError) {
      logger.error('⚠️  Failed to fix audit trigger:', triggerError.message);
    }
